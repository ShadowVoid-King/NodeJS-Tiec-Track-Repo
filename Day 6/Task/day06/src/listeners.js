...
bus.on("order:paid", (order) => {
	...
	if (order.status === "shipped" || order.status === "canceled") {
		return bus.emit("error", "Invalid transition to paid");
	}
...
bus.on("order:packed", (order) => {
	...
	if (order.status !== "paid")
		return bus.emit("error", "Pack requires status=paid");
...
bus.on("order:shipped", (order) => {
	...
	if (order.status !== "packed")
		return bus.emit("error", "Ship requires status=packed");

	if (order.status === "shipped")
		return bus.emit("error", "Order already shipped"); //! Change
...
bus.on("order:canceled", (order) => {
	...
	if (order.status === "shipped")
		return bus.emit("error", "Cannot cancel shipped order");
